...
from exp.exp_basic_multi import Exp_Basic
...
class Exp_Long_Term_Forecast_Multi(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_Multi, self).__init__(args)
        configs=args
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_text)[0]
                        else:
                            outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_text)
                else:
                    if self.args.output_attention:
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark, batch_text)[0]
                    else:
