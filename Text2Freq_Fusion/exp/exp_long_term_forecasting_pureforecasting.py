...
from exp.exp_basic_text2freq import Exp_Basic
from utils.tools import EarlyStopping_Fusion, EarlyStopping_Forecasting, adjust_learning_rate, visual, EarlyStopping
...
from models.Fusion import TTS, TTS_with_prior, TTS_concat
from tqdm import tqdm
from pre_utils.freq_utils import time_to_freq, freq_to_time, padding_for_lf
...
class Exp_Long_Term_Forecast_Pure(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_Pure, self).__init__(args)
        configs=args
        ...
        # revise the mlp after the Text2Freq module
        mlp_sizes=[self.args.series_length,self.args.pred_len]
        self.vq_output_length = configs.series_length
        self.use_freq = configs.use_freq
    ...
    def _build_fusion_model(self):
        fusion_model = TTS_concat(self.args.series_length, self.args.pred_len)
        return fusion_model

    def _build_vq_model(self):
        if self.args.use_freq == 0:
            vq_model = VQVAE(self.args.hidden_dim, self.args.series_length, self.args.n_embed, self.args.embed_dim, self.args.stride).to(self.device)
            vq_best_path = os.path.join(self.args.pretrain_path, f'vqvae/{self.args.type}/best_vqvae_model.pth')
            vq_model.load_state_dict(torch.load(vq_best_path,map_location=self.device))
        else:
            vq_model = VQVAE_freq(self.args.hidden_dim, self.args.series_length, self.args.n_embed, self.args.embed_dim, self.args.stride).to(self.device)
            vq_best_path = os.path.join(self.args.pretrain_path, f'vqvae/{self.args.type}/best_vqvae_model_freq_{self.args.lf}.pth')
            vq_model.load_state_dict(torch.load(vq_best_path,map_location=self.device))
        return vq_model
    
    def _build_trans_model(self):
        if self.args.use_freq == 0:
            trans_model = Transformer(text_embed_dim=512, n_layers=self.args.layers, n_heads=16, hidden_dim=self.args.hidden_dim, caption_length = self.args.caption_length, series_length=self.args.series_emb).to(self.device)
            trans_best_path = os.path.join(self.args.pretrain_path, f'pretrain/{self.args.type}/pretrain_text_series.pth')
            trans_model.load_state_dict(torch.load(trans_best_path,map_location=self.device))
        else:
            trans_model = Transformer(text_embed_dim=512, n_layers=self.args.layers, n_heads=16, hidden_dim=self.args.hidden_dim, caption_length = self.args.caption_length, series_length=self.args.series_emb).to(self.device)
            trans_best_path = os.path.join(self.args.pretrain_path, f'pretrain/{self.args.type}/pretrain_text_F{self.args.lf}.pth')
            trans_model.load_state_dict(torch.load(trans_best_path,map_location=self.device))
        return trans_model
    ...
    def _select_optimizer_weight(self):
        ...
        return model_optim
    def _select_optimizer_trans(self):
        optimizer = optim.AdamW(self.trans_model.parameters(), lr=1e-6)
        return optimizer
    def _select_optimizer_fusion(self):
        optimizer = optim.AdamW(self.fusion_model.parameters(), lr=0.001)
        return optimizer
    ...
    def vali_forecasting(self, vali_data, vali_loader, criterion):
        total_loss = []
        self.model.eval()
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                # convert to tuple
                batch_text = tuple(batch_text)
                batch_text = tuple(lst[0] for lst in batch_text)
                ...
                series_pred = outputs[:, -self.args.pred_len:, f_dim:]
    ...
    def train_forecasting(self, setting):
        ...
        early_stopping = EarlyStopping_Forecasting(patience=self.args.patience, verbose=True)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            #for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(tqdm(train_loader, desc="Training", leave=False)):
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):    
                iter_count += 1
                model_optim.zero_grad()
                batch_x = batch_x.float().to(self.device) # batch, seq_len, feature dim [32, 24, 1]
                batch_y = batch_y.float().to(self.device) # batch, seq_len, feature dim [32, 24, 1]
                #0523
                ...
                if self.args.use_amp:
                    ...
                    if self.args.output_attention:
                        ...
                        outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                f_dim = -1 if self.args.features == 'MS' else 0
                # print(f'out[ut shape {outputs.shape}') # batch size, pred_len, feature dim [32, 12, 1]
                series_pred = outputs[:, -self.args.pred_len:, f_dim:] 
            ...
            vali_loss = self.vali_forecasting(vali_data, vali_loader, criterion)
            test_loss = self.vali_forecasting(test_data, test_loader, criterion)
            ...
            if test_loss < best_test_loss:
                ...
                print(f'Current best test_loss {best_test_loss}')
    ...
    def test(self, setting, test=0):
        ...
        mae, mse, rmse, mape, mspe, dtw = metric(preds, trues)
