...
import re
from models.Fusion import TTS_TimeMMD
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_fusion_model(self):
        fusion_model = TTS_TimeMMD(self.args.pred_len, self.args.pred_len)
        return fusion_model
    ...
    def _select_optimizer_fusion(self):
        optimizer = optim.AdamW(self.fusion_model.parameters(), lr=self.args.learning_rate_fusion)
        return optimizer
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                outputs=self.fusion_model(outputs, prompt_y)
    ...
    def train(self, setting):
        ...
        model_optim_fusion = self._select_optimizer_fusion()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                prompt_y=norm(prompt_emb)
                # I have no idea what the origional code is writing about, fixing the weight params is not mentioned in their paper...
                outputs=self.fusion_model(outputs, prompt_y)
            ...
            early_stopping(vali_loss, self.model, self.fusion_model, path)
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            self.fusion_model.load_state_dict(torch.load(os.path.join('./checkpoints/' + setting, 'checkpoint_fusion.pth')))
